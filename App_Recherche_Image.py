...
def main():
    st.markdown("## Recherche des images similaires ")
                ...
                for row in range(num_rows):
                    ...
                    for i in range(images_per_row):
                        ...
                        if index < len(similar_images):
                            ...
                            img_path = os.path.join('./Images/Recherches', f'{label}.jpeg')

                            similar_img = Image.open(img_path)
                            if similar_img is not None:
                                cols[i].image(similar_img, caption=f"Distance: {distance:.2f}", width=image_width, use_column_width=False)
                            else:
